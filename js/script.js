...
if (pricingGrid) {
  gsap.fromTo('.price-card',
    { y: 50, opacity: 0 },
    { y: 0, opacity: 1, duration: 0.7, stagger: 0.18, ease: 'back.out(1.2)',
...
if (socialProofItems.length) {
  gsap.fromTo(socialProofItems,
    { y: 10, opacity: 0 },
    { y: 0, opacity: 1, duration: 0.4, stagger: 0.07, ease: 'power1.out',
...
// Target only the last section's CTA, not the hero CTA
const ctaSections = document.querySelectorAll('section');
const lastSection = ctaSections[ctaSections.length - 1];
if (lastSection) {
  gsap.fromTo(lastSection.children,
    { y: 20, opacity: 0 },
    { y: 0, opacity: 1, duration: 0.6, stagger: 0.12, ease: 'power2.out',
      scrollTrigger: { trigger: lastSection, start: 'top 85%', once: true } }
  );
}
